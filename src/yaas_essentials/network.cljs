...
(defn default-config [headers scopes]
  {:headers (merge headers (auth-header scopes)) :with-credentials? false}
  )

(defn yget
  ([url headers scopes reply-chan]
   (yget url headers scopes {} reply-chan)
    )
  ([url headers scopes params reply-chan]
